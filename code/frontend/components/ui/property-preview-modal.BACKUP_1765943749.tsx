...
import { X, MapPin, DollarSign, Home, Sparkles, TrendingUp, Users, Send, Target, ChevronRight, CheckCircle, Info, Calculator, Zap, Check, Crown, ArrowLeft, ArrowRight, Lock, FileText, MessageCircle, Image as ImageIcon, UserPlus, User, Building2, Star, AlertCircle, Layers, Edit, Save, Wrench } from 'lucide-react'
...
export default function PropertyPreviewModal({ isOpen, onClose, propertyData }: PropertyPreviewModalProps) {
  ...
  // ============================================================================
  // DEAL STRENGTH CALCULATION - Based on Spread % of ARV (Industry Standard)
  // ============================================================================
  
  // Calculate spread as percentage of ARV
  // Formula: spreadPercent = ((ARV - (sellerAsking + repairs + assignmentFee)) / ARV) × 100
  const calculateSpreadPercent = (): number => {
    if (!arv || arv === 0 || !sellerAsking || sellerAsking === 0) return 0
    const totalCosts = sellerAsking + (repairs || 0) + assignmentFee
    const spread = arv - totalCosts
    return (spread / arv) * 100
  }
  
  const spreadPercent = calculateSpreadPercent()
  const spreadDollar = arv && sellerAsking 
    ? Math.round(arv - (sellerAsking + (repairs || 0) + assignmentFee))
    : 0
  
  // Deal Strength Classification (Based on % of ARV)
  // STRONG → ≥ 20%
  // GOOD → 12–19.99%
  // SENSITIVE → 6–11.99%
  // RISKY → < 6%
  const getDealStrength = (): { grade: string; color: string; bgColor: string; borderColor: string; shadowColor: string } => {
    if (sellerAsking === 0) {
      return { 
        grade: 'RISKY', 
        color: 'text-red-400', 
  ...
  const dealStrength = getDealStrength()
  
  // Scenario analysis for AI Explanation section
  const spreadAtAsking = arv && sellerAsking 
    ? Math.round(arv - (sellerAsking + (repairs || 0) + assignmentFee))
    : 0
  const spreadAtMAO = arv && mao 
    ? Math.round(arv - (mao + assignmentFee + (repairs || 0)))
    : 0
  
  const qualityAtAsking = {
    grade: spreadAtAsking <= 0 ? 'RISKY' : spreadAtAsking < 25000 ? 'THIN' : 'SOLID',
    color: spreadAtAsking <= 0 ? 'red' : spreadAtAsking < 25000 ? 'amber' : 'emerald'
  }
  const qualityAtMAO = {
    grade: spreadAtMAO <= 0 ? 'RISKY' : spreadAtMAO < 25000 ? 'THIN' : 'SOLID',
    color: spreadAtMAO <= 0 ? 'red' : spreadAtMAO < 25000 ? 'amber' : 'emerald'
  }
  
  // Wholesaler's negotiation room: difference between MAO and asking
              ...
              <div className="space-y-6 animate-fade-in">
                ...
                {financing === 'cash' && (
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    {/* ARV Card */}
                    <div className="relative p-6 rounded-xl bg-gradient-to-br from-black via-gray-900 to-black border-2 border-amber-500/30 hover:border-amber-500/50 transition-all shadow-lg hover:shadow-amber-500/20">
                      <div className="flex items-center gap-2 mb-3">
                        <div className="p-2 rounded-lg bg-amber-500/10 border border-amber-500/30">
                          <Home className="w-5 h-5 text-amber-400" />
                        </div>
                        <p className="text-xs text-amber-300 font-montserrat font-bold uppercase tracking-wider">
                          AFTER-REPAIR VALUE
                        </p>
                      </div>
                      <p className="text-5xl font-montserrat font-black bg-gradient-to-r from-amber-400 via-yellow-400 to-amber-400 bg-clip-text text-transparent mb-2">
                        ${arv.toLocaleString()}
                      </p>
                    ...
                    <div className="relative p-6 rounded-xl bg-gradient-to-br from-black via-gray-900 to-black border-2 border-amber-500/30 hover:border-amber-500/50 transition-all shadow-lg hover:shadow-amber-500/20">
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <p className="text-xs text-amber-300 font-montserrat font-bold uppercase tracking-wider">MAX OFFER</p>
                      </div>
                      <p className="text-5xl font-montserrat font-black bg-gradient-to-r from-amber-400 via-yellow-400 to-amber-400 bg-clip-text text-transparent mb-2">
                        ${mao.toLocaleString()}
                      </p>
                    ...
                    {/* Repairs Card */}
                    <div className="relative p-6 rounded-xl bg-gradient-to-br from-black via-gray-900 to-black border-2 border-gray-700/30 hover:border-gray-700/50 transition-all shadow-lg">
                      <div className="flex items-center gap-2 mb-3">
                        <div className="p-2 rounded-lg bg-gray-600/10 border border-gray-600/30">
                          <Wrench className="w-5 h-5 text-gray-400" />
                        </div>
                        <p className="text-xs text-gray-300 font-montserrat font-bold uppercase tracking-wider">
                          REPAIRS
                        </p>
                      </div>
                    ...
                    {/* Deal Strength Card */}
                    <div className={`relative p-6 rounded-xl bg-gradient-to-br from-black via-gray-900 to-black border-2 transition-all shadow-lg ${
                      dealStrength.borderColor
                    } ${dealStrength.shadowColor}`}>
                      <div className="flex items-center gap-2 mb-3">
                        <div className={`p-2 rounded-lg border ${
                          dealStrength.bgColor
                        } ${dealStrength.borderColor.replace('border-', 'border-').replace('/40', '/30')}`}>
                          <Star className={`w-5 h-5 ${
                            dealStrength.color
                          }`} />
                        </div>
                        <p className="text-xs text-gray-300 font-montserrat font-bold uppercase tracking-wider">DEAL STRENGTH</p>
                      </div>
                      <p className={`text-4xl font-montserrat font-black mb-2 ${
                        dealStrength.color
                      }`}>
                        {dealStrength.grade}
                      </p>
                      <p className="text-xs text-gray-400 font-poppins">
                        {sellerAsking === 0 
                          ? 'Calculate to see spread' 
                          : `${spreadPercent.toFixed(1)}% spread • $${Math.abs(spreadDollar).toLocaleString()}`
                        }
                      </p>
                    </div>
                  </div>
                    ...
                    {activeTab === 'breakdown' && (
                      <div className="space-y-4">
                        {sellerAsking > 0 ? (
                          ...
                          <div className="p-6 rounded-xl bg-gradient-to-br from-amber-500/10 to-orange-500/10 border-2 border-amber-500/40">
                            <div className="flex items-start gap-4">
                              <AlertCircle className="w-6 h-6 text-amber-400 flex-shrink-0 mt-1" />
                              <div>
                                <h4 className="text-lg font-montserrat font-bold text-amber-300 mb-2">
                                  ⚠️ Enter Seller Asking Price in Step 3
                                </h4>
                                <p className="text-sm text-gray-300 font-poppins leading-relaxed">
                                  The complete deal breakdown will appear here once you enter the seller's asking price. This helps calculate your negotiation room and deal spread.
                                </p>
                              </div>
                            </div>
                          </div>
