...
import { X, MapPin, DollarSign, Home, Sparkles, TrendingUp, Users, Send, Target, ChevronRight, CheckCircle, Info, Calculator, Zap, Check, Crown, ArrowLeft, ArrowRight, Lock, FileText, MessageCircle, Image as ImageIcon, UserPlus, User, Building2, Star, AlertCircle, Layers } from 'lucide-react'
...
export default function PropertyPreviewModal({ isOpen, onClose, propertyData }: PropertyPreviewModalProps) {
  ...
  const handleCalculate = () => {
    if (!repairs) {
      toast.error('Please generate rehab estimate first')
      return
    }
    setCurrentStep(4)
  ...
  const handleCalculateChecklist = () => {
    ...
    // Close repair mode and advance to analysis results
    setRepairMode(null)
    setCurrentStep(4)
                    ...
                    <div className="mb-6">
                      <button
                        onClick={() => {
                          ...
                          setRepairs(0)
                          setRepairMode(null)
                          setCurrentStep(4) // Advance to next step
                          setTimeout(() => {
                            const modalContent = document.querySelector('.overflow-y-auto')
                            if (modalContent) {
                              modalContent.scrollTo({ top: modalContent.scrollHeight, behavior: 'smooth' })
